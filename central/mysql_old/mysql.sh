...
function mysql_command() {
    sudo docker exec -it "$CONTAINER_NAME_CENTRAL" /bin/bash -c "mysql --defaults-extra-file=$my_cnf --verbose --execute $1"
}

function mysql_command_return() {
    result=$(sudo docker exec -it "$CONTAINER_NAME_CENTRAL" /bin/bash -c "mysql --defaults-extra-file=$my_cnf --verbose --execute $1")
