...
if ! "$JALIEN_PATH/jalien" -e cp file://"$SAMPLE_JDL_PATH" alien://sample.jdl >/dev/null; then
    if "$JALIEN_PATH/jalien" -e ls | grep -q "sample.jdl"; then
...
level="Critical"
description="Sample JDL should be copied to SE before submitting the job"
if ! "$JALIEN_PATH/jalien" -e cp file://$"$TESTSCRIPT_PATH" alien://testscript.sh >/dev/null; then
    if "$JALIEN_PATH/jalien" -e ls | grep -q "testscript.sh"; then
