...
for package in "${packages[@]}"; do
    ...
    sudo docker exec "$container_name" yum list installed "$package" > /dev/null 2>&1

    if [[ $? -eq 0 ]]; then
        echo -e "${success_color}Package '$package' is installed in container '$container_name'${reset_color}"
    else
        echo -e "${error_color}Package '$package' is not installed in container '$container_name'${reset_color}"
    fi
done


# Check whether Zulu java is installed or not
sudo docker exec "$container_name" yum list installed | grep zulu11.68.17-ca-jdk11.0.21-linux.x86_64 > /dev/null 2>&1
if [[ $? -eq 0 ]]; then
    echo -e "${success_color}Package 'zulu11.68.17-ca-jdk11.0.21-linux.x86_64' is installed in container '$container_name'${reset_color}"
else
    echo -e "${error_color}Package 'zulu11.68.17-ca-jdk11.0.21-linux.x86_64' is not installed in container '$container_name'${reset_color}"
fi
