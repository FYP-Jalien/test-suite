...
level="Critical"
if docker ps --format '{{.Names}}' | grep -qw "$CONTAINER_NAME_WORKER"; then
    status="PASSED"
...
level="Warning"
actual_image=$(docker inspect --format='{{.Config.Image}}' "$CONTAINER_NAME_WORKER")
...
level="Warning"
actual_volumes=$(docker inspect --format='{{range .Mounts}}{{.Source}}:{{.Destination}}:{{.Mode}} {{end}}' "$CONTAINER_NAME_WORKER" )
...
level="Warning"
actual_command=$(docker inspect --format='{{.Config.Cmd}}' "$CONTAINER_NAME_WORKER" )
...
level="Warning"
actual_environment=$(docker inspect --format='{{range $key, $value := .Config.Env}}{{$value}} {{end}}' "$CONTAINER_NAME_WORKER" )t
...
level="Minor"
actual_hostname=$(docker inspect --format='{{.Config.Hostname}}' "$CONTAINER_NAME_WORKER" )
