...
expected_lines=(
    ...
    "Mar 27, 2024 10:35:21 AM alien.catalogue.IndexTableEntry getDB"
    "Host is : Host: hostIndex: [0-9]\+"
    "address	: localhost:3307"
    "database	: alice_users"
    "driver	: mysql"
...
expected_lines=(
    ...
    " We received parameters: {Site=JTestSite, Partition=,,, CE=ALICE::JTestSite::firstce, Platform=Linux-x86_64, Host=localhost.localdomain, Users=\[\], TTL=86396, alienCm=localhost.localdomain:10000, AliEnPrincipal=\[jobagent\], CheckedLDAP=, workdir=/var/lib/condor/execute/dir_[0-9]\+, NoUsers=\[\], Localhost=worker1, CEhost=localhost.localdomain, Disk=[0-9]\+, CPUCores=[0-9], CVMFS=1}"
...
expected_lines=(
    ...
    "owner		: jalien:admin"
    "ctime		: Wed Mar [0-9]\+ [0-9:]\+ UTC [0-9]\+ (expires null)"
    "replicated	: false"
    "aclId		: 0"
